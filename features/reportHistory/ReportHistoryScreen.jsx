  ...
  router.push({
    ...
    params: {
      match: JSON.stringify({
        ...match,
        lost_item_image: report.lost_item_image,
        lost_item_name: report.lost_item_name,
        lost_report_id: report.lost_report_id,
          ...
          >
            ...
            <Text
              style={[
                ...
                !online && styles.disabledButtonText,
              ]}
            >
              Edit Report
            </Text>
          ...
          {report.status === "open" && (
            <>
              ...
              <TouchableOpacity
                style={[
                  styles.cancelButton,
                  !online && styles.disabledCancelButton,
                ]}
                activeOpacity={0.7}
                disabled={!online}
                onPress={onCancel}
...
export default function ReportHistoryScreen() {
  ...
  const [activeMenu, setActiveMenu] = useState(savedFilters?.activeMenu ?? null);
  ...
  const confirmCancel = async (reason) => {
    if (!reportToCancel || !online) return;
    setCancelModalVisible(false);

    try {
      await cancelLostReport(reportToCancel.lost_report_id, reason);
      setReports((prev) =>
        prev.map((r) =>
          ...
          <View style={styles.filterBar}>
            <View style={styles.filterContainer}>
              ...
              <TouchableOpacity
                style={[
                  styles.filterTrigger,
                  (selectedStatuses.length > 0 || activeMenu === "status") &&
                    styles.filterTriggerActive,
                ]}
                onPress={() =>
