...
export default function ReportNextPage() {
  ...
  // ── RE-LOAD DRAFT & LOCATION DATA EVERY TIME PAGE 2 GAINS FOCUS ──
  useFocusEffect(
    useCallback(() => {
      ...
      // Update draft state immediately to dismiss spinner
      setDraft(saved);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // ── 1. PURGE MODULE DRAFT IN MEMORY FIRST ──
      clearReportDraft();

      // ── 2. RESET LOCAL PAGE 2 STATES ──
      setDraft(null);
      setDate(new Date());
      setTime(new Date());
      setCantRemember(false);
