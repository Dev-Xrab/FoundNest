...
import { buildLocationLost, submitLostReport, validateReportPage2 } from "@/utils/lostReport";
...
const CustomCheckbox = ({ label, value, onValueChange }) => (
  <TouchableOpacity style={styles.checkboxContainer} onPress={() => onValueChange(!value)} activeOpacity={0.7}>
    <MaterialIcons name={value ? "check-box" : "check-box-outline-blank"} size={22} color={value ? AppColors.background : "#757575"} />
    <Text style={styles.checkboxLabel}>{label}</Text>
  </TouchableOpacity>
);

const NestedDropdownHeader = ({ title, isOpen, disabled, onPress }) => (
  <TouchableOpacity onPress={onPress} disabled={disabled} style={[styles.nestedHeader, disabled && styles.nestedHeaderDisabled]} activeOpacity={0.7}>
    <Text style={[styles.nestedHeaderTitle, disabled && styles.disabledText]}>{title}</Text>
    <MaterialIcons name={isOpen ? "keyboard-arrow-down" : "keyboard-arrow-right"} size={22} color="#900014" />
  </TouchableOpacity>
);
...
export default function ReportNextPage() {
  ...
  useEffect(() => {
    ...
    if (!saved) {
      Alert.alert("Incomplete form", "Please complete page 1 before continuing.", [
  ...
  const formatDataList = (sourceData) => {
    return Array.isArray(sourceData) ? sourceData.map((item) => (item && item.name ? item.name : item)) : [];
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // ── 1. WIPE SYSTEM STORAGE CACHE HOOKS ──
      clearReportDraft();

      // ── 2. FLUSH INTERACTIVE PAGE 2 COMPONENT DATA ──
      setDate(new Date());
      setTime(new Date());
      setCantRemember(false);
      setSelectedColleges([]);
      ...
      // ── 3. REPLACE STACK OVERLAY WITH SUCCESS SCREEN ──
      router.replace({
      ...
      Alert.alert("Submission failed", error?.message ?? "Could not submit your report.");
    } finally {
  ...
  return (
    <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={styles.container}>
      <DatePicker modal open={openCalendar} date={date} mode="date" maximumDate={new Date()} onConfirm={(d) => { setOpenCalendar(false); setDate(d); }} onCancel={() => setOpenCalendar(false)} />
      <DatePicker modal open={openClock} mode="time" date={time} onConfirm={(t) => { setOpenClock(false); setTime(t); }} onCancel={() => setOpenClock(false)} />
      ...
      <ScrollView ref={scrollRef} keyboardShouldPersistTaps="handled" contentContainerStyle={styles.scrollContent}>
        ...
        <TouchableOpacity onPress={() => setOpenCalendar(true)} activeOpacity={0.8}>
        ...
        <TouchableOpacity onPress={() => setOpenClock(true)} activeOpacity={0.8}>
          <View style={styles.dataPickerButton}>
              ...
              {openSubSection === "colleges" && (
                <View style={styles.nestedCheckboxList}>
                  {formatDataList(collegesList).map((item, idx) => (
                    <CustomCheckbox key={`college-${idx}`} label={item} value={selectedColleges.includes(item)} onValueChange={() => handleLocationSelectionChange(selectedColleges, setSelectedColleges)(item)} />
                  ))}
                </View>
              )}
              ...
              {openSubSection === "spaces" && (
                <View style={styles.nestedCheckboxList}>
                  {formatDataList(spacesList).map((item, idx) => (
                    <CustomCheckbox key={`space-${idx}`} label={item} value={selectedSpaces.includes(item)} onValueChange={() => handleLocationSelectionChange(selectedSpaces, setSelectedSpaces)(item)} />
                  ))}
                </View>
              )}
              ...
              {openSubSection === "gates" && (
                <View style={styles.nestedCheckboxList}>
                  {formatDataList(gatesList).map((item, idx) => (
                    <CustomCheckbox key={`gate-${idx}`} label={item} value={selectedGates.includes(item)} onValueChange={() => handleLocationSelectionChange(selectedGates, setSelectedGates)(item)} />
                  ))}
                </View>
              )}

              <TouchableOpacity style={[styles.nestedHeader, styles.cantRememberRow]} onPress={handleCantRememberChange} activeOpacity={0.7}>
        ...
        <View style={styles.nextSection}>
          ...
          <View style={styles.buttonSection}>
            <TouchableOpacity style={styles.cancelButton} disabled={isSubmitting} onPress={() => router.back()}>
...
const styles = StyleSheet.create({
  ...
  infoCard: { backgroundColor: "#E3D5CA", borderRadius: 8, padding: 16, marginHorizontal: 20, marginTop: 28, marginBottom: 12 },
